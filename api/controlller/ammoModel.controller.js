...
exports.getAmmoModel = async (req, res) => {
	let weapons = {};
	try {
		weapons = await WeaponAmmunition.findAll({
			where: { ammoModelID: req.params.ammoModelID },
...
exports.updateAmmoModel = async (req, res) => {
	...
	try {
		if (ammoModel.hasOwnProperty('WeaponModels')) {
			weaponModels = await WeaponAmmunition.bulkCreate(ammoModel.WeaponModels, {
		...
		ammoModel = ammoModel.dataValues;
		weaponModels = weaponModels.map((item) => converter(item.dataValues));
...
exports.deleteWeaponAmmunition = async (req, res) => {
	try {
		await WeaponAmmunition.destroy({
			where: { ammoModelID: req.params.ammoModelID, weaponModelID: req.params.weaponModelID },
		});
		return res.status(200).send('Succesfully ammoModel deleted');
	} catch (e) {
		return res.status(400).send(e.message);
