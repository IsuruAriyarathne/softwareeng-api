...
exports.updateCompany = async (req, res) => {
	...
	try {
		if (company.hasOwnProperty('SupplyAmmunition')) {
			supplyAmmunition = await SupplyAmmunition.bulkCreate(company.SupplyAmmunition, {
		...
		if (company.hasOwnProperty('SupplyWeapon')) {
			supplyWeapon = await SupplyWeapon.bulkCreate(company.SupplyWeapon, {
				ignoreDuplicates: true,
				transaction: t,
			});
		}
...
exports.deleteAmmunitionTypes = async (req, res) => {
	try {
		await SupplyAmmunition.destroy({
			where: { supplierID: req.params.supplierID, ammoModelID: req.params.ammoModelID },
		});
...
exports.deleteWeaponModel = async (req, res) => {
	try {
		await SupplyWeapon.destroy({
			where: { supplierID: req.params.supplierID, weaponModelID: req.params.weaponModelID },
		});
